...
io.on('connection_error', (error: any): void => {
  logger.error(
    `Connection error: ${socketErrorText(error.code)} - ${error.message}`,
  );
});
...
// middleware
userNamespaceIo.use(
  async (client: Socket | any, next: any): Promise<string | void> => {
    const token = client.handshake.auth.token;
    if (!token) {
      return next(
        new BadRequestError('Invalid token. Please pass a valid token.'),
      );
    }
    ...
    if (user === null) {
      return next(
        new BadRequestError(
          'Invalid token provided. Please pass a valid token.',
        ),
      );
    }
    client.user = user;
...
userNamespaceIo.on(
  'connection',
  async (authClient: Socket | any): Promise<void> => {
    const userId = authClient.user.id;
    logger.info(`Authenticated client: ${userId} connected.`);

    const notifications: any = await redisGetBatchRecords(
      config.cache.notificationsSentKey,
    );
    if (notifications.length) {
      notifications.forEach((data: any) => {
        if (data.users.indexOf(userId) === -1) {
          prepareMissedNotification(data.notification, userId).then(
            (notification) => {
              if (notification)
                authClient.emit(
                  config.socket.events.notification.generated,
                  notification,
                );
            },
          );
    ...
    authClient.join([
      socketGroup.feeds,
      socketGroup.categories,
      socketGroup.users,
      socketGroup.notifications,
    ]);
...
export { httpServer, io, express, app, pubClient, subClient, userNamespaceIo };
