...
export const registerSocialProfile = async (
  profile: any,
  ...
  try {
    ...
    if (error.code && error.code === 11000) {
      logger.info(
        `${strategy}Strategy - Duplicate value entered for ${Object.keys(
          error.keyValue,
        )} field.`,
      );
      if (strategy === 'default') {
        throw new CustomAPIError(
          `This ${Object.keys(error.keyValue)} is already in use.`,
        );
      }
...
const transformFacebookData = (profile: FacebookProfile) => {
  ...
  const displayNames = profile.displayName.length
