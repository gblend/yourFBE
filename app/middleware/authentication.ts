...
const authenticateUser = async (
  ...
  try {
    if (!accessToken && authorization && authorization.startsWith('Bearer')) {
    ...
    if (accessToken) {
      ...
      if (!isTokenExist || !isTokenExist?.isValid) {
        ...
        res
          ...
          .json({ message: constants.auth.AUTHENTICATION_INVALID });
      }

      const accessTokenJWT = createJWT(payload.user);
      attachCookiesToResponse({
        accessTokenJWT,
        refreshTokenJWT: isTokenExist!.refreshToken,
        res,
      });
    ...
    return next(
      new UnauthenticatedError(constants.auth.AUTHENTICATION_INVALID),
    );
  } catch (err: any) {
