...
router.route('/disabled-accounts').get(authenticateUser, authorizeRoles(admin), getDisabledAccounts);
...
router.route('/accounts/moderate/:id').delete(authenticateUser, authorizeRoles(admin, user), disableUserAccount)
...
router.route('/update/account').patch(authenticateUser, authorizeRoles(admin, user), updateUser);
