...
const getCategories = async (req: Request, res: Response): Promise<void> => {
    ...
    const cachedAllCategories: any[] = await redisGetBatchRecords(categoriesCacheKey);
    if (cachedAllCategories.length) {
        categories = cachedAllCategories;
    } else {
        const allCategories = FeedCategory.find({status: constants.STATUS_ENABLED}).populate('feedsCount');
        if (sort) {
    ...
    const {pagination, result: categoriesResult} = await paginate(categories, {pageSize, pageNumber});
...
const getCategoryById = async (req: Request, res: Response) => {
    ...
    if (cachedCategory) {
        ...
        category = await FeedCategory.findById(categoryId, {status: constants.STATUS_ENABLED}).populate('feeds',
...
const updateCategory = async (req: Request, res: Response): Promise<void> => {
    ...
    const updatedCategory = await FeedCategory.findOneAndUpdate({_id: categoryId}, body, {
        runValidators: true,
        new: true
    });
    if (!updatedCategory) {
