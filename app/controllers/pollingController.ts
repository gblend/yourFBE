...
class RSSFeedPollingQueue {
  ...
  constructor(
    client: (feedData: feed) => Promise<any>,
    feeds: any[] = [],
...
export const pollFeeds = async (): Promise<void> => {
  logScheduledProcess(feedsPollingCronCacheKey).then(
    async (scheduleRunning: boolean): Promise<void> => {
      const processScheduled: boolean = JSON.parse(
        await redisGet(feedsPollingCronCacheKey),
      );
      ...
      if (!processScheduled) {
        ...
        if (feeds.length) {
          const feedsIds: string[] = feeds.map(
            (feedData: feed): string => feedData._id,
          );
          logger.info(
            `${feeds.length} feeds data retrieved successfully, proceeding to poll feeds posts.`,
          );
