...
export const logScheduledProcess = async (code: string = '', completed: boolean = false): Promise<boolean> => {
    const session: ClientSession = await mongoose.connection.startSession();
    const lockEntry: any = {scheduledLock: {scheduledCode: code, pseudoRandom: new mongoose.Types.ObjectId()}};
    ...
    try {
        ...
        if (!completed) {
            const processInfo = await Schedule.findOneAndUpdate({code},
                {$set: lockEntry}, {upsert: false, new: false});
            ...
            if (processInfo && processInfo.status === 'running') {
                ...
                const processCreated = await Schedule.findOneAndUpdate({code},
            ...
            const processCompleted = await Schedule.findOneAndUpdate({code},
                {status: 'completed', ended_at: new Date(Date.now()), $set: lockEntry}, {new: true});
