...
const getAllUsers = async (
  ...
  const { pagination, result } = await paginate(usersQuery, {
    pageSize,
    pageNumber,
  });
  ...
  if (!usersData.length) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - No user found for get_all_users - ${method} ${_path}`,
    );
...
const getDisabledAccounts = async (
  req: Request,
  res: Response,
): Promise<Response<IResponse>> => {
  const {
  ...
  if (!users.length) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - No user found for get_disabled_accounts - ${method} ${_path}`,
    );
    throw new NotFoundError('No disabled account found');
  }

  const { pagination, result } = await paginate(users, {
    pageSize,
    pageNumber,
  });
...
const getAllAdmins = async (
  ...
  if (admins.length < 1) {
    admins = await User.find({ role: constants.role.ADMIN }).select([
      '-password',
      '-verificationToken',
    ]);
    if (admins.length < 1) {
      logger.info(
        JSON.stringify(
          `${StatusCodes.NOT_FOUND} - No admin found for get_all_admins - ${method} ${_path}`,
        ),
      );
  ...
  const { pagination, result } = await paginate(admins, {
    pageSize,
    pageNumber,
  });
  logger.info(
    `${StatusCodes.OK} - ${JSON.stringify(admins)} - ${method} ${_path}`,
  );
  return res.status(StatusCodes.OK).json({
...
const getUser = async (
  ...
  if (!mongoose.isValidObjectId(userId)) {
    logger.info(
      `${StatusCodes.BAD_REQUEST} ${constants.auth.INVALID_CREDENTIALS(
        'user id',
      )} for get_user - ${method} ${_path}`,
    );
  ...
  if (!user) {
    logger.info(
      JSON.stringify(
        `${StatusCodes.NOT_FOUND} - User with id ${userId} does not exist for get_single_user - ${method} ${_path}`,
      ),
    );
  ...
  logger.info(
    `${StatusCodes.OK} - Account details fetched successfully - ${method} ${_path}`,
  );
  return res
    .status(StatusCodes.OK)
    .json({ message: 'Account details fetched successfully', data: { user } });
};
...
const showCurrentUser = async (
  ...
  if (!id) {
    logger.info(
      `${StatusCodes.UNAUTHORIZED} - Unauthorized access for show_current_user - ${method} ${_path}`,
    );
  ...
  if (!userData) {
    logger.info(
      JSON.stringify(
        `${StatusCodes.NOT_FOUND} - User with id ${id} does not exist for show_current_user - ${method} ${_path}`,
      ),
    );
...
const updateUser = async (
  ...
  account = await User.findOneAndUpdate({ _id: userId }, body, {
    new: true,
    runValidators: true,
  });
  if (!account) {
    logger.info(
      JSON.stringify(
        `${StatusCodes.NOT_FOUND} - User with id ${userId} does not exist for update_account - ${method} ${_path}`,
      ),
    );
...
const disableUserAccount = async (
  ...
  const account: any = await User.findById(userId).select([
    '-password',
    '-verificationToken',
  ]);
  ...
  if (!account) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - No account found with id ${userId} - ${method} ${path}`,
    );
    throw new NotFoundError(`Account not found`);
  }
  checkPermissions(user, account._id);
  ...
  return res.status(StatusCodes.OK).json({
    message:
      user.role === 'admin'
        ? `${account.firstname} ${account.lastname}'s account disabled successfully`
...
const enableUserAccount = async (
  ...
  const account: any = await User.findOneAndUpdate(
    { _id: userId },
    { status: constants.STATUS_ENABLED },
