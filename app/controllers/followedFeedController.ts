...
const followFeed = async (req: Request, res: Response) => {
  ...
  if (!isFollowed) {
    ...
    logger.info(
      `${StatusCodes.OK} - Followed feed ${body.feed} successfully - ${method} ${path}`,
    );
  }

  res.status(StatusCodes.CREATED).json({
    message: 'Feed followed successfully.',
    data: { followed: followedFeed },
  });
};
...
const followAllFeedsInCategory = async (req: Request, res: Response) => {
  ...
  const { error } = validateFollowFeedsInCategoryDto({
    user: _user,
    category: categoryId,
  });
  if (error) {
  ...
  const categoryFeedsData = await Feed.find(
    { category: categoryId, status: 'enabled' },
    { _id: 1 },
  )
  ...
  try {
    const followCategoryFeeds = await FollowedFeed.insertMany(
      _categoryFeedsData,
      { ordered: false },
    );
    if (!followCategoryFeeds) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: `Unable to follow ${categoryName} category feeds` });
    }

    logger.info(
      `${StatusCodes.OK} - Successfully followed ${categoryName} category feeds  - ${method} ${path}`,
    );
    res.status(StatusCodes.OK).json({
      message: `Successfully followed ${categoryName} category feeds`,
    });
  } catch (err: any) {
...
const unfollowAllFeedsInCategory = async (req: Request, res: Response) => {
  ...
  const unfollowedFeeds = await FollowedFeed.deleteMany({
    user: _user,
    category: categoryId,
  });
  ...
  logger.info(
    `${StatusCodes.OK} - All feeds in category: ${categoryId} unfollowed successfully - ${method} ${path}`,
  );
  ...
  res
    ...
    .json({ message: 'All feeds in this category unfollowed successfully.' });
};

const unfollowFeed = async (req: Request, res: Response) => {
  const {
    pathParams: { id: followedFeedId },
    method,
    path,
  } = adaptRequest(req);
  ...
  const deleted = await FollowedFeed.findOneAndDelete({
    _id: createObjectId(followedFeedId),
  });
  if (!deleted) {
    throw new BadRequestError(
      `Followed feed ${followedFeedId} does not exist.`,
    );
  }

  logger.info(
    `${StatusCodes.OK} - Feed unfollowed successfully - ${method} ${path}`,
  );
...
const unfollowAllFeeds = async (req: Request, res: Response) => {
  ...
  logger.info(
    `${StatusCodes.OK} - All feeds unfollowed successfully - ${method} ${path}`,
  );
  ...
  res
    .status(StatusCodes.OK)
    .json({ message: 'All feeds unfollowed successfully.' });
};
...
const latestPostsByFollowedFeeds = async (req: Request, res: Response) => {
  ...
  const followFeeds: { feed: objectId }[] = await FollowedFeed.find({
    user: userId,
  }).select('feed -_id');
  if (!followFeeds.length) {
    logger.info(`No followed feed found`);
    throw new NotFoundError(
      `No followed feed found. Please follow available feeds.`,
    );
  }

  const cachedFeedPostsId: any[] = followFeeds.map(
    (followed: { feed: any }) => {
      if (followed.feed)
        return config.cache.feedPostsKey + `_${followed.feed.toLocaleString()}`;
    },
  );
  const feedPosts = await redisMGet(cachedFeedPostsId);
  let followedFeedPosts: any[] = await Promise.all(
    feedPosts.map((feedData: any): any =>
      transformRssFeed(feedData, { pageSize, pageNumber }),
    ),
  );

  const { pagination, result } = await paginate(followedFeedPosts, {
    pageSize,
    pageNumber,
  });
  ...
  logger.info(
    `${StatusCodes.OK} - Latest posts from followed feeds fetched successfully. - ${method} ${path}`,
  );
  res.status(StatusCodes.OK).json({
...
const getFollowedFeeds = async (req: Request, res: Response) => {
  ...
  const followedFeeds = FollowedFeed.find({
  ...
  const { pagination, result } = await paginate(followedFeeds, {
    pageSize,
    pageNumber,
  });
  ...
  if (!followedFeedsResult.length) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - No followed feeds found for get_followed_feeds - ${method} ${path}`,
    );
    throw new NotFoundError('No followed feed found.');
  }
  logger.info(
    `${StatusCodes.OK} - Followed feeds fetched successfully - ${method} ${path}`,
  );
  return res.status(StatusCodes.OK).json({
...
const feedsFollowersStats = async (req: Request, res: Response) => {
  const {
    path,
    method,
    queryParams: { pageSize: _pageSize, pageNumber: _pageNumber },
  } = adaptRequest(req);
  const { pageSize, pageNumber, offset } = adaptPaginateParams(
    _pageSize,
    _pageNumber,
  );
  ...
  if (
    !feedsFollowersResult[0]?.paginatedResults.length ||
    !feedsFollowersResult[0]?.feeds.length
  ) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - No feeds followers found - ${method} ${path}`,
    );
    throw new NotFoundError('No feeds followers found');
  }
  const { result, total, pages, next, previous } = mapPaginatedData(
    feedsFollowersResult,
    pageSize,
    pageNumber,
  );

  logger.info(
    `${StatusCodes.OK} - All feeds followers stats fetched successfully - ${method} ${path}`,
  );
  res.status(StatusCodes.OK).json({
