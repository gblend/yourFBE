...
import { ActivityLog, PollingLog } from '../models';
import { Request, Response } from '../types';

const getActivityLogs = async (req: Request, res: Response) => {
  const {
    queryParams: { pageSize, pageNumber, sort, fields },
    method,
    path,
  } = adaptRequest(req);
  ...
  const { pagination, result } = await paginate(activityLogs, {
    pageSize,
    pageNumber,
  });
  ...
  if (!logs.length) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - Activity logs not found. - ${method} ${path}`,
    );
    return res
      .status(StatusCodes.NOT_FOUND)
      .json({ message: `Activity logs not found.` });
  }
...
const getActivityLog = async (req: Request, res: Response) => {
  ...
  if (!activityLog) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - Activity log with id ${logId} does not exist. - ${method} ${path}`,
    );
    return res
      .status(StatusCodes.NOT_FOUND)
      .json({ message: `Activity log with id ${logId} does not exist.` });
  }
...
const getPollingLog = async (req: Request, res: Response) => {
  ...
  if (!pollingLog) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - Polling log with id ${logId} does not exist. - ${method} ${path}`,
    );
    return res
      .status(StatusCodes.NOT_FOUND)
      .json({ message: `Polling log with id ${logId} does not exist.` });
  }
...
const getPollingLogs = async (req: Request, res: Response) => {
  const {
    method,
    path,
    queryParams: { sort, fields, pageSize, pageNumber },
  } = adaptRequest(req);
  ...
  const { pagination, result } = await paginate(pollingLogs, {
    pageSize,
    pageNumber,
  });
  const logs = await result;
  ...
  if (!logs.length) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - Polling logs no found. - ${method} ${path}`,
    );
    return res
      .status(StatusCodes.NOT_FOUND)
      .json({ message: `Polling logs not found.` });
  }
  res.status(StatusCodes.OK).json({
    message: 'Activity logs fetched successfully.',
    data: {
...
const searchLogs = async (req: Request, res: Response) => {
  const {
    queryParams: { type = 'polling', searchTerm, pageSize, pageNumber },
    method,
    path,
  } = adaptRequest(req);
  ...
  const { pagination, result } = await paginate(searchResult, {
    pageSize,
    pageNumber,
  });
  const logs = await result;
  ...
  if (!logs.length) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - No result found for ${type} logs search. - ${method} ${path}`,
    );
    return res
      .status(StatusCodes.NOT_FOUND)
      .json({ message: `No result found for ${type} logs search.` });
  }
  res.status(StatusCodes.OK).json({
