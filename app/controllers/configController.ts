...
const getAllConfig = async (req: Request, res: Response) => {
  ...
  if (!configData.length) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - No config data found for get_all_config - ${method} ${path}`,
    );
    throw new NotFoundError('No config data found.');
  }
  const { pagination, result } = await paginate(configData, {
    pageSize,
    pageNumber,
  });
...
const getSingleConfig = async (
  req: Request,
  res: Response,
): Promise<Response<IResponse>> => {
  const {
    pathParams: { id: configId },
    path,
    method,
  } = adaptRequest(req);
  ...
  if (!config) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - Config with id ${configId} not found for get_single_config - ${method} ${path}`,
    );
...
const getConfigByPath = async (
  req: Request,
  res: Response,
): Promise<Response<IResponse>> => {
  const {
    queryParams: { reference: configPath },
    path,
    method,
  } = adaptRequest(req);
  ...
  if (!config) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - Config with path ${configPath} not found for get_config_by_path - ${method} ${path}`,
    );
...
const updateConfig = async (
  ...
  const config = await ConfigData.findOneAndUpdate({ _id: configId }, body, {
    new: true,
    runValidators: true,
  });
  const logData = {
  ...
  return res
    ...
    .json({ message: 'Config updated successfully.', data: { config } });
};
...
const disableConfig = async (
  ...
  await ConfigData.findOneAndUpdate(
    { _id: configId },
...
const deleteConfig = async (
  ...
  if (!config) {
    logger.info(
      `${StatusCodes.NOT_FOUND} Config with id ${configId} not found for delete_config - ${method} ${path}`,
    );
    throw new BadRequestError(`Config not found.`);
  }
  await config.remove();
