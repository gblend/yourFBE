...
const uploadProfileImage = async (
  ...
  const {
    files,
    body: { uploadType = 'cloudinary', folder = '' },
    user,
    path,
    method,
  } = adaptRequest(req);
  ...
  const { avatar }: any = await User.findOneAndUpdate(
    { _id: createObjectId(user.id) },
    { avatar: url },
  ...
  if (!avatar) {
    logger.info(
      JSON.stringify(
        `${StatusCodes.NOT_FOUND} - Account not found for upload_profile_image - ${method} ${path}`,
      ),
    );
    throw new NotFoundError(
      `Account not found for profile image upload action`,
    );
  }

  logger.info(
    `${StatusCodes.OK} - Profile image upload successful - ${method} ${path}`,
  );
