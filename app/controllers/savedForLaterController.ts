...
const savePostForLater = async (req: Request, res: Response) => {
  ...
  if (!isSavedForLater) {
    ...
    if (!Object.keys(savedPost).length) {
      throw new BadRequestError(
        `Unable to save post for later. Please try again later.`,
      );
    }
    await redisSetBatchRecords(
      `${config.cache.savePostForLaterCacheKey}_${userId}_${savedPost._id}`,
      [savedPost],
    );
  ...
  userNamespaceIo
    .to('savedForLater')
    .volatile.emit('admin:post_starred', { post: savedPost, userId });
  res.status(StatusCodes.OK).json({
    message: 'Post successfully saved for later.',
    data: { savedForLater: savedPost },
  });
};
...
const deletePostSavedForLater = async (req: Request, res: Response) => {
  ...
  if (!postId || !mongoose.isValidObjectId(postId)) {
    logger.info(
      `${StatusCodes.BAD_REQUEST} - Invalid post id: ${postId} - ${method} ${path}`,
    );
    throw new BadRequestError('Invalid post id.');
  }

  const deleted = await SavedForLater.findOneAndDelete({
    _id: postId,
    user: userId,
  });
  if (!deleted) {
  ...
  await redisRefreshCache(
    `${config.cache.savePostForLaterCacheKey}_${userId}_${postId}`,
  );
  ...
  userNamespaceIo
    .to('savedForLater')
    .volatile.emit('admin:post_deleted', { post: deleted, userId });
  res
    .status(StatusCodes.OK)
    .json({ message: 'Post saved for later successfully removed.' });
};
...
const getPostsSavedForLater = async (req: Request, res: Response) => {
  ...
  let savedPosts: any = await redisGetBatchRecords(
    `${config.cache.savePostForLaterCacheKey}_${userId}`,
  );
  if (savedPosts && savedPosts.length) {
    logger.info(
      `${StatusCodes.OK} - Posts saved for later retrieved from cache - ${method} ${path}`,
    );
    const { pagination: _pagination, result: _result } = await paginate(
      savedPosts,
      { pageSize, pageNumber },
    );
  ...
  const { pagination, result } = await paginate(savedPosts, {
    pageSize,
    pageNumber,
  });
  ...
  if (!savedPostsData || !savedPostsData.length) {
    return res
      ...
      .json({ message: 'No saved posts found.' });
  }
...
const getPostSavedForLater = async (req: Request, res: Response) => {
  ...
  if (!postId) {
    logger.info(
      `${StatusCodes.BAD_REQUEST} - Invalid post saved for later id: ${postId} - ${method} ${path}`,
    );
    throw new BadRequestError('Invalid post saved for later id');
  }

  let post: any = await redisGetBatchRecords(
    `${config.cache.savePostForLaterCacheKey}_${userId}_${postId}`,
  );
  if (!post.length) {
    ...
    await redisSetBatchRecords(
      `${config.cache.savePostForLaterCacheKey}_${userId}_${post._id}`,
      [post],
    );
  }
...
const markPostSavedForLaterAsRead = async (req: Request, res: Response) => {
  ...
  const post = await SavedForLater.findOneAndUpdate(
    { _id: postId },
    { status: 'read' },
    {
      runValidators: true,
  ...
  if (post) {
    ...
    logger.info(
      `${StatusCodes.OK} - Post saved for later marked as read - ${method} ${path}`,
    );
    return res.status(StatusCodes.OK).json({
  ...
  res
    .status(StatusCodes.NOT_FOUND)
    .json({ message: 'Post saved for later not found.' });
};
...
const userStarredFeedPostsStats = async (req: Request, res: Response) => {
  ...
  if (
    !starredFeedPostsStat[0]?.paginatedResults.length ||
    !starredFeedPostsStat[0]?.feeds.length
  ) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - No user starred feed posts found - ${method} ${path}`,
    );
    throw new NotFoundError('No user starred feed posts found');
  }
  const { result, total, pages, next, previous } = mapPaginatedData(
    starredFeedPostsStat,
    pageSize,
    pageNumber,
  );

  logger.info(
    `${StatusCodes.OK} - User starred feed posts stats fetched successfully - ${method} ${path}`,
  );
  res.status(StatusCodes.OK).json({
...
const allStarredFeedPostsStats = async (req: Request, res: Response) => {
  ...
  if (
    !starredFeedPostsStats[0]?.paginatedResults.length ||
    !starredFeedPostsStats[0]?.feeds.length
  ) {
    logger.info(
      `${StatusCodes.NOT_FOUND} - No starred feed posts found - ${method} ${path}`,
    );
  ...
  logger.info(
    `${StatusCodes.OK} - All starred feed posts stats fetched successfully - ${method} ${path}`,
  );
  res.status(StatusCodes.OK).json({
