...
const createFeed = async (req: Request, res: Response) => {
    ...
    const {requestUrl: url, requestProtocol: protocol} = formatUrlProtocol(body.url);
    return protocol.get(url, async (response: any): Promise<any> => {
        ...
        if (statusCode === 301) {
            ...
            return res.status(StatusCodes.MOVED_PERMANENTLY).json(
                {message: `Feed url moved permanently. Please use new url: ${newUrl}`, data: {oldURL: url, newUrl}});
        }
    }).on('error', (err: Error): any => {
        logger.error(`Feed url: ${url} is invalid or inactive.`, err);
        return res.status(StatusCodes.BAD_REQUEST).json({message: 'Feed url is invalid or inactive.'});
    }).end();
}
...
const getPostsByFeedId = async (req: Request, res: Response) => {
    const {path, method, pathParams: {id: feedId}, queryParams: {postSize = 10, pageNumber = 1}} = adaptRequest(req);
    if (!feedId || !mongoose.isValidObjectId(feedId)) {
    ...
    if (!feedPosts) {
        const feedDetails: any = await Feed.findOne({_id: feedId}).select('_id, url');
        if (!feedDetails) {
    ...
    const transformedFeed = await transformRssFeed(feedPosts, {postSize, pageNumber});
...
const randomFeedsPosts = async (req: Request, res: Response): Promise<Response> => {
    const {path, method, queryParams: {limit = 10, postSize = 10, pageNumber = 1}} = adaptRequest(req);
    ...
    const cachedFeedsId: string[] = feeds.map((feed: { _id: string }): string =>
        config.cache.feedPostsKey + `_${feed._id.toLocaleString()}`);

    const feedPosts = await redisMGet(cachedFeedsId);
    const result: any[] = await Promise.all(feedPosts.map((feed: any): any =>
        transformRssFeed(feed, {postSize, pageNumber}).then((feedData: any) => {
            delete feedData.pagination;
            return feedData;
        })));

    logger.info(`${StatusCodes.OK} - Random ${result.length} feed posts fetched successfully. - ${method} ${path}`);
    return res.status(StatusCodes.OK).json({data: {randomFeedsPosts: result}});
}

const recentFeedPosts = async (req: Request, res: Response): Promise<Response> => {
    const {path, method, queryParams: {limit = 10, postSize = 1, pageNumber = 1}} = adaptRequest(req);

    const feeds: { _id: objectId }[] = await Feed.find({status: constants.STATUS_ENABLED}).limit(limit).select('_id');

    const cachedFeedsId: string[] = feeds.map((feed: { _id: string }): string =>
        config.cache.feedPostsKey + `_${feed._id.toString()}`);

    const cachedFeeds = await redisMGet(cachedFeedsId);
    const feedPosts: any[] = await Promise.all(cachedFeeds.map((feed: any): any =>
    ...
    logger.info(`${StatusCodes.OK} - Recent ${result.length} feed posts fetched successfully. - ${method} ${path}`);
