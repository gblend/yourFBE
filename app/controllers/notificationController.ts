...
import {validateNotificationDto, Notification, validateUpdateNotificationDto} from '../models/Notification';
...
const createNotification = async (req: Request, res: Response) => {
	...
	const notification = await Notification.findOneAndUpdate({title: body.title, text: body.text},
		body, {upsert: true, new: true});

	if (!notification) {
		throw new BadRequestError(`Unable to create notification. Please try again later.`);
	}
	...
	userNamespaceIo.to(notificationRoom).emit(notificationEvent.generated, {notification});
	// get comma separated string of all active sockets id and save them into notification log
	if (activeSocketIds) {
		const notificationLogData = {users: activeSocketIds, notification: createObjectId(notification._id) as objectId}
		const {error: errors} = validateNotificationLogDto(notificationLogData);
		if (errors) {
			logger.info(JSON.stringify(JSON.stringify(formatValidationError(errors))));
			return res.status(StatusCodes.BAD_REQUEST).json({data: {errors: formatValidationError(errors)}});
		}

		const newNotification = await NotificationLog.findOneAndUpdate({notification: notificationLogData.notification},
			notificationLogData, {upsert: true, new: true});

		if (newNotification) {
			const notifications = await NotificationLog.find({}).select('users _id notification').lean();
			// save all sent notifications data in redis for faster lookup
			await redisSetBatchRecords(config.cache.notificationsSentCacheKey, notifications, 0, true);
		}
	}

	res.status(StatusCodes.OK).json({message: 'Notification created and sent successfully.', data: {notification}});
}
...
const getNotifications = async (req: Request, res: Response) => {
	...
	if (!notificationsResult.length) {
		logger.info(`${StatusCodes.NOT_FOUND} - No notification found for get_notifications - ${method} ${path}`);
		throw new NotFoundError('No notification found.');
	}
	logger.info(`${StatusCodes.OK} - Notifications fetched successfully - ${method} ${path}`);
	return res.status(StatusCodes.OK).json({message: 'Notifications fetched successfully.',
...
const deleteNotification = async (req: Request, res: Response) => {
	...
	if (!deletedNotification) {
		throw new BadRequestError('Notification was not deleted. Please confirm notification id is valid.');
	}
	...
	await saveActivityLog(logData);
	logger.info(`${StatusCodes.OK} - Notification deleted successfully - ${method} ${path}`);

	// @TODO: remove from redis notifications_id
	userNamespaceIo.to(notificationRoom).emit(notificationEvent.deleted, {notificationId});
...
const updateNotification = async (req: Request, res: Response) => {
	...
	const notification = await Notification.findOneAndUpdate({_id: notificationId}, body,
		{runValidators: true, new: true});
	if (!notification) {
		throw new BadRequestError('Notification update failed. Please confirm notification id is valid.');
	}
	const logData = {
	...
	await saveActivityLog(logData);
	logger.info(`${StatusCodes.OK} - Notification updated successfully - ${method} ${path}`);

	userNamespaceIo.to(notificationRoom).emit(notificationEvent.updated, {notification});
	return res.status(StatusCodes.OK).json({message: 'Notification updated successfully.', data: {notification}});
}

const prepareMissedNotification = async (notificationId: objectId, userId: objectId) => {
	notificationId = createObjectId(notificationId);
	const notification = await Notification.findById(notificationId);

	if (notification) {
		const notificationLog = await NotificationLog.findOne({notification: notificationId});
