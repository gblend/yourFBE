...
const search = async (req: Request, res: Response) => {
  const {
    ...
    path,
    body: { searchTerm },
    queryParams: { sort, pageNumber, pageSize },
  } = adaptRequest(req);
  if (!searchTerm) {
  ...
  if (!searchResult.length) {
    logger.info(
      `${StatusCodes.NOT_FOUND} No result found for: ${searchTerm} - ${method} - ${path}`,
    );
    throw new NotFoundError(`No result found`);
  }

  const { result, pagination } = await paginate(searchResult, {
    pageSize,
    pageNumber,
  });
  logger.info(
    `${StatusCodes.OK} Search results retrieved successfully - ${method} - ${path}`,
  );
