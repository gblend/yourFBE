...
const register = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (isEmailExist) {
    throw new CustomAPIError(
      constants.auth.ALREADY_IN_USE('This email address'),
    );
  }
  ...
  role === ADMIN
    ? await redisRefreshCache(config.cache.adminsKey)
    : await redisRefreshCache(config.cache.usersKey);
  ...
  const refreshTokenJWT: string = await user.createRefreshJWT(
    user,
  ...
  userNamespaceIo
    .to(config.socket.group.users)
    .volatile.emit(config.socket.events.admin.userRegistered, { user });
...
const login = async (req: Request, res: Response) => {
  ...
  if (user.socialChannelId && !user.password) {
    throw new BadRequestError(
      'This account can only be accessed via social login',
    );
  }
  ...
  await user.save();
  const tokenInfo = await saveTokenInfo(user, { ip, headers });
  const refreshTokenJWT = await user.createRefreshJWT(
    user,
    tokenInfo.refreshToken,
  );
...
const socialLogin = async (req: Request, res: Response) => {
  const {
    ...
    ip,
    socialProfile,
    body: { profileData, updateProfile = false },
  } = adaptRequest(req);
  let user: any;
  ...
  if (updateProfile) {
    ...
    user = await registerSocialProfile(profileData, (_: any, __: any) => {
      /*unused */
    });
...
const logout = async (req: Request, res: Response): Promise<void> => {
  const {
    user,
    signedCookies,
  }: { user: ITokenUser | any; signedCookies: any } = adaptRequest(req);
...
const forgotPassword = async (req: Request, res: Response): Promise<void> => {
  ...
  if (user) {
    ...
    await pushToQueue(queueName, queueErrorMsg, resetPasswordPayload).catch(
      (_: Error) => useCallback(sendResetPasswordEmail, resetPasswordPayload),
    );

    const passwordTokenExpirationDate: Date = new Date(
      Date.now() + config.minutes.ten,
    );
...
const resetPassword = async (req: Request, res: Response): Promise<void> => {
  ...
  if (!email || !token || !password) {
    throw new BadRequestError(
      'Please provide all values - email, token, password',
    );
  }
...
const saveTokenInfo = async (
  { _id: userId }: { _id?: ObjectId },
  { ip, headers }: { ip: string; headers: any },
) => {
