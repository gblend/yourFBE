import { ActivityLog, validateActivityLogDto } from '../models/ActivityLog';
import { PollingLog, validatePollingLogDto } from '../models/PollingLog';
...
const saveActivityLog = async (activityLog: IActivityDto): Promise<void> => {
  ...
  if (!error) {
    ...
    logger.info(
      `${StatusCodes.OK} - ActivityLog saved - ${activityLog.method} ${activityLog.path}`,
    );
  }
};
...
const savePollingLog = async (pollingLog: IPollingDto): Promise<void> => {
  ...
  if (!error) {
    ...
    logger.info(
      `${StatusCodes.OK} - ActivityLog saved - ${pollingLog.method} ${pollingLog.path}`,
    );
  }
};

export { savePollingLog, saveActivityLog };
