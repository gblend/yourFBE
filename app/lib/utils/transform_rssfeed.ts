...
export const transformRssFeed = async (feedData: any, page: pageInfo): Promise<IRssFeed> => {
    const feed = (typeof feedData === 'string') ? JSON.parse(feedData) : feedData;
    const {result, pagination} = await paginate(feed.item, {pageSize: page.postSize, pageNumber: page.pageNumber});
