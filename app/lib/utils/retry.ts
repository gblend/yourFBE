...
export const request = (
  {
    ...
    method = 'GET',
    url: webAddress,
  }: { _id?: string; method?: string; url: string },
  cb: (data: any, feedId?: string) => any,
): boolean => {
  const { requestUrl: url, requestProtocol: protocol } =
    formatUrlProtocol(webAddress);
...
export const formatUrlProtocol = (
  url: string,
): { requestUrl: string; requestProtocol: any } => {
